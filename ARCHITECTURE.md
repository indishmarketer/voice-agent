...
Everything after hangup — summarising, lead extraction, Sheets sync — is fired
into a detached task so it never touches the call's latency.
...
SQLite with WAL, on the `/app/data` volume. Tables: `visitors`, `sessions`,
`turns`, `leads`. IP addresses are stored only as a keyed HMAC hash.
